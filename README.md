...
- [Operator](https://github.com/iishyfishyy/operator-oss): Local-first control room that runs many Claude Code or Codex sessions in parallel across projects — each task is its own session in an isolated git worktree, with diff review, branch sync and merge in the UI; driven by your existing agent login, no API key. ![GitHub Repo stars](https://img.shields.io/github/stars/iishyfishyy/operator-oss?style=social)
